...
CREATE POLICY "Users can read their own pipeline jobs"
    ...
    USING (
        EXISTS (
            ...
            WHERE p.id = pipeline_jobs.paper_id AND p.user_id::text = get_current_user_id()::text
        )
    );
...
CREATE POLICY "Users can read their own paper chunks"
    ...
    USING (
        EXISTS (
            SELECT 1 FROM public.papers p 
            WHERE p.id = paper_chunks.paper_id AND p.user_id::text = get_current_user_id()::text
        )
    );
